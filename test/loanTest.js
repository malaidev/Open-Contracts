...
describe("===== Loan Test =====", function () {
    ...
    before(async function () {
        ...
        diamondAddress = await deployDiamond()
        // diamondAddress = "0xA69A505Ffe4A42a8715cF9d98d1034dfa7e28566";
    ...
    it("Check borrow", async () => {
        await expect(loan1.connect(contractOwner).loanRequest(symbolUsdt, comit_ONEMONTH, 100, symbolUsdt, 50, {gasLimit: 5000000}))
            .to.emit(library, "NewLoan")
    })

    it("Check addCollateral", async () => {
        await expect(loan1.connect(contractOwner).addCollateral(symbolUsdt, comit_ONEMONTH, symbolUsdt, 20, {gasLimit: 5000000}))
            .to.emit(library, "AddCollateral")
    })
    ...
    it("Swap Loan", async () => {
        await expect(loan.connect(contractOwner).swapLoan(symbolUsdt, comit_ONEMONTH, symbolBtc, {gasLimit: 5000000}))
            .to.emit(library, "SwapTestAmount");
    ...
    it("Check repayLoan", async () => {
        await (loan.connect(contractOwner).repayLoan(symbolUsdt, comit_ONEMONTH, 30, {gasLimit: 5000000}));
    })
