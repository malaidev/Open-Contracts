1. Check if the loan exists & is active.
1.	update apr
2.	parse repayAmount to the


_repaymentProcess()
1.	AppStorageOpen storage ds = diamondStorage(); 
2.	uint _remnantAmount  = 0;
3.	uint256 collateralAmount = collateral.amount - _accruedAPR
4.	if (commitment == _getcommitment[2]) collateralAmount += _accruedAPY
5.	_repaymentAmount += collateralAmount._swap;
6.	if (_repaymentAmount >= loan.amount)  uint _remnantAmount = repayAmount minus loan.amount
7.	else { if (loanState.currentMarket == loan.actualLoanMarket) _repayAmount +=  loanState.currentAmount
1.	else loanState.currentMarket swap
2.	_repayAmount +=  loanState.currentAmount
3.	_remnantAmount = repayAmount minus loan.amount
4.	return _remnantAmount;

repayLoan()
1.	if (commitment == _getcommitment[2]) 
    1.	convert _remnantAmount to collateral.Amount
    2.	delete APR -  done
    3.	delete APY
    4.	update LoanRecords, LoanState, LoanAccount
2.	if (commitment == _getcommitment[0]) 
    1.	convert _remnantAmount to collateral.Amount
    2.	delete APR
    3.	delete APY
    4.	transfer funds
    5.	delete LoanRecords, LoanState, LoanAccount

liquidation
1.	delete APR
2.	delete APY
3.	delete LoanRecords, LoanState, LoanAccount
4.	_updateReserves
