...
contract Loan is Pausable, ILoan {
	...
	function swapLoan(
		...
		return LibDiamond._swapLoan(_market, _commitment, _swapMarket);
	}
	...
	function collateralPointer(address _account, bytes32 _market, bytes32 _commitment, bytes32 collateralMarket, uint collateralAmount) external view{
		...
		LibDiamond.LoanRecords storage loan = ds.indLoanRecords[_account][_market][_commitment];
		LibDiamond.LoanState storage loanState = ds.indLoanState[_account][_market][_commitment];
		LibDiamond.CollateralRecords storage collateral = ds.indCollateralRecords[_account][_market][_commitment];
	...
	function repayLoan(bytes32 _market,bytes32 _commitment,uint256 _repayAmount) external  returns (bool success) {
		return LibDiamond._repayLoan(_market, _commitment, _repayAmount);
	}
	...
	function permissibleWithdrawal(bytes32 _market,bytes32 _commitment, bytes32 _collateralMarket, uint256 _amount) external returns (bool success) {
		return LibDiamond._permissibleWithdrawal(_market, _commitment, _collateralMarket, _amount);
		
	}

	function liquidation(address _account, uint256 _id) external nonReentrant()	authLoan() returns (bool success) {
